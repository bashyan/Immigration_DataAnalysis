...
public class momap 
{
	...
	{	
		...
		public void map(LongWritable key, Text value, Context context)throws IOException, InterruptedException 
		{
			...
			if(age > 14)
			{
				...
				context.write(new LongWritable(0),new Text("TaxAll\t"+taxcal));
				
				if(citizen.contains(" Foreign"))
				{
					context.write(new LongWritable(1), new Text("ForeignTax\t"+taxcal+","+filer));
				}
	...
	public static class medianMapper extends Mapper<LongWritable, Text, LongWritable, Text> 
	{
		...
		public void map(LongWritable key, Text value, Context context)throws IOException, InterruptedException 
		{
			...
			context.write(new LongWritable(3), new Text("IncomeAll\t"+incval));
			if(citizen.contains(" Foreign"))
			{
				context.write(new LongWritable(4), new Text("ForeignIncome\t"+incval));
			}
			if(citizen.contains(" Native"))
			{
		...
		public void reduce(LongWritable key, Iterable<Text> values, Context context)throws IOException, InterruptedException 
		{
			...
			for (Text entry : values) 
			{
				...
				else if (parts[0].equals("ForeignTax")) 
				{
					...
					{
						taxnonpayforeign += Double.parseDouble(rows[0]);
					}
				} 
